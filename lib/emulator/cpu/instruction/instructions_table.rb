  ...
  module Cpu
    module Instruction
      class InstructionsTable
        ...
        def initialize
          byte_sized_instructions = INSTRUCTIONS.reject {|instruction| instruction.instruction_id.opcode > 0xFF}
          word_sized_instructions = INSTRUCTIONS.reject {|instruction| instruction.instruction_id.opcode <= 0xFF}

          @byte_sized_instructions_table = Array.new(0xFF)
          byte_sized_instructions.each {|instruction| @byte_sized_instructions_table[instruction.instruction_id.opcode] = instruction}

          @word_sized_instructions = Array.new(0xFF)
          word_sized_instructions.each {|instruction| @word_sized_instructions[instruction.instruction_id.opcode & 0xFF] = instruction}
        end
